    ...
    def get(self, sale_order_date=None):
        ...
        try:
            if sale_order_date:
                ...
                # start_date = SalesItems.query.filter_by(SalesItems.sale_date==start).first()
                # end_date = SalesItems.query.filter_by(sale_date=end).first()
                #
                # if not start_date:
                #     return {"error" : "Please enter valid start date"}
                # if not end_date:
                #     return {"error" : "Please enter valid end date"}
                ...
                count = SalesItems.query.filter(and_(SalesItems.sale_date >= start, SalesItems.sale_date <= end)).count()
